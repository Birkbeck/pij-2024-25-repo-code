...
public class DoublyLinkedList<E> implements SimpleList<E> {
     ...
     */
    public DoublyLinkedList(SimpleList<? extends E> copyMe) {
     ...
     * @throws NullPointerException if copyMe is the null reference
     */
    public DoublyLinkedList(DoublyLinkedList<? extends E> copyMe) {
        ...
        DLLNode<? extends E> currentCopyMe = copyMe.head; // the next node to copy
        E data = currentCopyMe.getData();
        this.head = new DLLNode<>(data); // first element set
        DLLNode<E> currentThis = this.head; // the current end node in the new list
    ...
    public E remove(int index) {
        ...
        E result = this.head.remove(index);
        if (index == 0) {
